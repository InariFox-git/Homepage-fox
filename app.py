...
@app.route('/get_folders/<user_id>', methods=['GET'])
def get_folders(user_id):
    users = load_users()
    if user_id in users:
        return jsonify(users[user_id].get('folders', []))
    return jsonify([])

@app.route('/save_folders/<user_id>', methods=['POST'])
def save_folders_route(user_id):
    folders = request.json
    users = load_users()
